...
class CrossCompressUnit(nn.Module):
    ...
    def forward(self, inputs: list, inputs_0:list):
        # [batch_size, dim]
        v, e = inputs
        v0, e0 =inputs_0
        ...
        # [batch_size, dim, dim]
        c_matrix_1 = v.matmul(e0).permute([0, 2, 1]).contiguous()
        c_matrix_2 = v0.matmul(e)
        

        # [batch_size * dim, dim]
        c_matrix_1 = c_matrix_1.view(-1, self.dim)
        c_matrix_2 = c_matrix_2.view(-1, self.dim)

        # F.linear(input, weight, bias=None)
        # [batch_size, dim]
        v_out = F.linear(c_matrix_1, self.ev)
        v_out = v_out.view(-1, self.dim) + v.view(-1, self.dim)+ self.bias_v

        e_out = F.linear(c_matrix_2, self.ve)
        e_out = e_out.view(-1, self.dim) + e.view(-1, self.dim)+ self.bias_e
